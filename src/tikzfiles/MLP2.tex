...
\node[textonly, above right=38pt and -86pt of h1] (f) {$\vect{\hat{y}} = f(\vect{x};\vect{\theta})= g_2(\vect{U}\vect{h})= g_2(\vect{U}g_1(\vect{W}\vect{x}))$};
...
\path[tedge] (h1) edge node[above=1.8pt] {$\vect{U}_{1,1}$} (z1);
\path[tedge] (h2) edge node[below left=-4.2pt and 4.5pt] {$\vect{U}_{2,1}$} (z1);

\path[tedge] (h1) edge node[above=1.8pt] {$\vect{U}_{1,2}$} (z2);
\path[tedge] (h2) edge node[above=0.2pt] {$\vect{U}_{2,2}$} (z2);
